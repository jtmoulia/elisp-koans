...
(elisp-koans/define-test test-basic-loop
    (let* ((letters '(:a :b :c :d))
           (loop-result
...
(elisp-koans/define-test test-compound-loop
    (let* ((letters '(:a :b :c :d))
           (loop-result
             (cl-loop for letter in letters
...
(elisp-koans/define-test test-counting-loop-skip-by-syntax
...
(elisp-koans/define-test test-counting-backwards
    (let ((loop-result
...
(elisp-koans/define-test test-loop-in-vs-loop-on
    (let* ((letters '(:a :b :c))
           (loop-result-in
...
(elisp-koans/define-test test-loop-in-skip-by
...
(elisp-koans/define-test test-loop-over-hash-tables
    (let* ((pairs-in-table
...
(elisp-koans/define-test test-value-accumulation-forms
    (let ((loop-1
...
(elisp-koans/define-test test-destructuring-bind
    (let* ((count 0)
...
(elisp-koans/define-test test-conditional-execution
    (let ((loop-return
...
(elisp-koans/define-test test-conditional-with-defun
    (let ((loop-return
           (cl-loop for x in '(1 1 2 3 5 8 13)
...
(elisp-koans/define-test test-conditional-with-lambda
    (let ((loop-return
           (cl-loop for x in '(1 1 2 3 5 8 13)
